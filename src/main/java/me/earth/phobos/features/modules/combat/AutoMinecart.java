...
extends Module {
    ...
    public void onUpdate() {
        ...
        Vec3d hitVec = new Vec3d((Vec3i)pos).add(0.0, -0.5, 0.0);
    ...
    private EntityPlayer getTarget() {
        ...
        for (EntityPlayer player : AutoMinecart.mc.world.playerEntities) {
            if (EntityUtil.isntValid((Entity)player, 6.0) || player.isInWater() || player.isInLava() || !EntityUtil.isTrapped(player, false, false, false, false, false, false) || player.getHealth() + player.getAbsorptionAmount() > this.minHP.getValue().floatValue()) continue;
            if (target == null) {
                target = player;
                distance = AutoMinecart.mc.player.getDistanceSq((Entity)player);
                continue;
            }
            if (!(AutoMinecart.mc.player.getDistanceSq((Entity)player) < distance)) continue;
            target = player;
            distance = AutoMinecart.mc.player.getDistanceSq((Entity)player);
        }
        return target;
