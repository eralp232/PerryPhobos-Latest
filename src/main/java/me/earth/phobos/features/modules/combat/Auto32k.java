...
extends Module {
    ...
    private void placeDispenserAgainstBlock(BlockPos dispenserPos, BlockPos helpingPos) {
        ...
        EnumFacing facing = EnumFacing.DOWN;
        for (EnumFacing enumFacing : EnumFacing.values()) {
            BlockPos blockPos = dispenserPos.offset(enumFacing);
        ...
        if (this.rotate.getValue().booleanValue()) {
            if (this.blocksPerPlace.getValue() > 1) {
                float[] fArray = RotationUtil.getLegitRotations(hitVec);
                if (this.extra.getValue().booleanValue()) {
                    RotationUtil.faceYawAndPitch(fArray[0], fArray[1]);
                }
            } else {
                this.rotateToPos(null, hitVec);
            }
            ...
            float[] fArray = RotationUtil.simpleFacing(facings);
            this.yaw = fArray[0];
            this.pitch = fArray[1];
            this.spoof = true;
        } else {
        ...
        Vec3d rotationVec = new Vec3d((Vec3i)helpingPos).add(0.5, 0.5, 0.5).add(new Vec3d(opposite.getDirectionVec()).scale(0.5));
        float[] fArray = RotationUtil.simpleFacing(facings);
        float[] angle = RotationUtil.getLegitRotations(hitVec);
        if (this.superPacket.getValue().booleanValue()) {
            RotationUtil.faceYawAndPitch(this.rotate.getValue() == false ? fArray[0] : angle[0], this.rotate.getValue() == false ? fArray[1] : angle[1]);
        }
    ...
    private DispenserData findBestPos() {
        ...
        switch (type) {
            ...
            case ENEMY: {
                positions.sort(Comparator.comparingDouble(arg_0 -> ((EntityPlayer)this.target).getDistanceSq(arg_0)));
                break;
            }
            case MIDDLE: {
                ArrayList<BlockPos> toRemove = new ArrayList<BlockPos>();
