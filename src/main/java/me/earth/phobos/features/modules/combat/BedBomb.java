...
extends Module {
    ...
    private void mapBeds() {
        ...
        if (this.removeTiles.getValue().booleanValue()) {
            ...
            for (BedData data : removedBlocks) {
                ...
                if (!(BedBomb.mc.player.getDistanceSq(pos) <= MathUtil.square(this.breakRange.getValue().floatValue())) || !((double)(selfDamage = DamageUtil.calculateDamage(pos, (Entity)BedBomb.mc.player)) + 1.0 < (double)EntityUtil.getHealth((Entity)BedBomb.mc.player)) && DamageUtil.canTakeDamage(this.suicide.getValue())) continue;
    ...
    private void placeBed(BlockPos pos, boolean firstCheck) {
        ...
        for (EnumFacing facing : EnumFacing.values()) {
            ...
            if (!(BedBomb.mc.player.getDistanceSq(position) <= MathUtil.square(this.placeRange.getValue().floatValue())) || !BedBomb.mc.world.getBlockState(position).getMaterial().isReplaceable() || BedBomb.mc.world.getBlockState(position.down()).getMaterial().isReplaceable()) continue;
