...
extends Module {
    ...
    public Map<EntityPlayer, TimerUtil> playerTimers = new ConcurrentHashMap<EntityPlayer, TimerUtil>();
    ...
    public static void displayInv(ItemStack stack, String name) {
        try {
            ...
            ItemStackHelper.loadAllItems((NBTTagCompound)Objects.requireNonNull(stack.getTagCompound()).getCompoundTag("BlockEntityTag"), (NonNullList)entityBox.items);
    ...
    public void onUpdate() {
        ...
        for (EntityPlayer player : ToolTips.mc.world.playerEntities) {
            if (player == null || !(player.getHeldItemMainhand().getItem() instanceof ItemShulkerBox) || EntityUtil.isFakePlayer(player) || !this.own.getValue().booleanValue() && ToolTips.mc.player.equals((Object)player)) continue;
