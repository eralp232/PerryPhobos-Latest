...
extends Module {
    ...
    public Bypass() {
        super("Bypass", "Bypass's for stuff.", Module.Category.MISC, true, false, false);
        instance = this;
    }
    ...
    public void onIncomingPacket(PacketEvent.Receive event) {
        if (!Bypass.fullNullCheck() && this.elytra.getValue().booleanValue()) {
            ...
            if (this.cancel_close.getValue().booleanValue() && Bypass.mc.player.isElytraFlying() && event.getPacket() instanceof SPacketEntityMetadata && ((SPacketEntityMetadata)event.getPacket()).getEntityId() == Bypass.mc.player.getEntityId()) {
                event.setCanceled(true);
            }
        }
